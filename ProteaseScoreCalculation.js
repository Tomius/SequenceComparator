class ProteaseScoreCalculation {
    ...
    The_Calculation(PTC, POI, MinScore, MinSelec) {
        ...
        const Ratio_By_AA_And_Position = Array.from({ length: 20 }, () => Array(8).fill(0));
        const Max_By_Position = Array(8).fill(0);
        const Value_At_The_Protease_Of_Interest = Array(8).fill(0);
        const AA_By_Position = Array(8).fill('');
        const Closest_Protease_by_Position = Array(8).fill('');
        ...
        for (let k = 0; k < 8; k++) {
            ...
            const Max_By_AA = Array(20).fill(0);
            ...
            for (let i = 0; i < 20; i++) {
                if (Values_By_AA[this.Index_POI][i] >= MinScore) {
                    ...
                    Ratio_By_AA_And_Position[i][k] = Max_By_AA[i] !== 0 
                        ? parseFloat((Values_By_AA[this.Index_POI][i] * 100 / Max_By_AA[i]).toFixed(2)) 
                        : 0;

                    if (Ratio_By_AA_And_Position[i][k] > MinSelec) {
                        if (Ratio_By_AA_And_Position[i][k] > Max_By_Position[k]) {
                            Max_By_Position[k] = Ratio_By_AA_And_Position[i][k];
                                        ...
                                        if (new_sequence.every(val => val !== '')) {
                                            ...
                                            if (mean_sequence >= inputMinScore) {
                                                combinations.push({
                                                    ...
                                                    'MeanScore': mean_sequence.toFixed(2)
                                                });
                                                index++;
                                            }
                                        }
